...
class BaseAPLC:
    def __init__(self, amp, amp_dx, efl, wvl, dark_hole, dh_dx, fpm, ls,
                 ...
                 cost_function, include_fpm, alpha, symmetry):
        """
        ...
        symmetry: str
            Symmetries to employ when constructing the optimization. Options
            are "point" and "quadrant".

            Point symmetry assumes f(x) = f(-x). This is useful for annular /
            mirror symmetric focal planes to substantially reduce the
            computational complexity. For this preliminary implementation, we
            will always assume that the user supplies the the apodizer on the
            **RIGHT** side of the array. If symmetery="point", it will be
            copied to the left half of the array.

            Quadrant symmetry assumes that each quadrant is a mirror image of
            the one adjacent to it, and reduces the complexity further than
            point symmetry. For this preliminary implementation, we will always
            assume the the user supplies the apodizer on the **TOP RIGHT** of
            the array, or "quadrant 1", using the following convention
                |
             2  |  1
                |
            ---------
            ...
            Q1 is supplied by the user. Q2 = np.fliplr(Q1), Q4 = np.flipud(Q1)
            Q3 = np.fliplr(Q4)
        ...
        if self.symmetry == "point":
            ...
            # Go up to half the apodizer shape
            self.amp_select[:, :self.apodizer_shape[0] // 2] = 0

        elif self.symmetry == "quadrant":

            # Q2 and Q3 are zeroed
            self.amp_select[:, :self.apodizer_shape[0] // 2] = 0

            # Zero Q4
            self.amp_select[self.apodizer_shape[0] // 2:,
                            self.apodizer_shape[0] // 2:] = 0
    ...
    def update(self, x):
        ...
        assert self.symmetry in ["point", "quadrant"]
        ...
        if self.symmetry == "point":
            ...
            Q1 = np.copy(self.aplc)

            # Q2
            self.aplc += np.fliplr(Q1)

            # Q3
            self.aplc += np.flipud(Q1)

            # Q4
            self.aplc += np.fliplr(np.flipud(Q1))
...
class PhaseAPLC(BaseAPLC):
    ...
    def __init__(self, amp, amp_dx, efl, wvl, dark_hole, dh_dx, fpm, ls,
                 basis=None, initial_amplitude=None, activation=None, weight=1,
                 include_fpm=True, symmetry=False):
        ...
        super().__init__(self, amp, amp_dx, efl, wvl, dark_hole, dh_dx, fpm, ls,
                       ...
                       include_fpm=include_fpm, symmetry=symmetry)
...
class ThroughputOptimizer:
    ...
    def __init__(self, amp, wvl, basis, ls, initial_amplitude=None, center_wavelength=None, relative_weight=1, symmetry=None):
        if initial_amplitude is None:
            aplc = np.zeros(amp.shape, dtype=np.float64)
        ...
        self.symmetry = symmetry
        self.apodizer_shape = self.amp.shape
