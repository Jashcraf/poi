...
for color, BANDWIDTH, MULTIPLIER, FAILED in zip(colors, BANDWIDTHS, MULTIPLIERS, FAILED_SOLUTION):
    ...
    # LP mask path
    mask_path_lp = Path.home() / "Downloads/Data_LPBandwidth" / f"APLC_{BANDWIDTH}_2026-09-11/apodizer_0.35cenwvl_5wvls_{BANDWIDTH}%.fits"
    newmask_lp = np.array(fits.getdata(mask_path_lp))
    ...
    for i, ld in tqdm(enumerate(tilt_lds)):
        ...
        if FAILED == 1:
            plt.scatter(BANDWIDTH, throughput_07, marker="o", linestyle="None", c=colors[0], s=50)
        else:
            plt.scatter(BANDWIDTH, throughput_07, marker="x", linestyle="None", c=colors[1], s=50)
...
plt.legend()
plt.savefig("throughput_bandwidths.png")
plt.show()
