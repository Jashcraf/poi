...
ax6.legend()
plt.savefig(f"iwa_survey/LUVOIR_B_{PUPIL_NPIX}Npup_{IMG_NPIX}Nimg_{IWA}IWA_{OWA}OWA_{LS_FRAC}LS_{THROUGHPUT_LOG_WEIGHT}throughput_weight.pdf")

# Save apodizer as .fits
newmask = newmask.get()
fits.writeto(f"iwa_survey/LUVOIRB_{PUPIL_NPIX}Npup_{IMG_NPIX}Nimg_{IWA}IWA_{OWA}OWA_{LS_FRAC}LS_{THROUGHPUT_LOG_WEIGHT}throughput_weight.fits", newmask, overwrite=True)
