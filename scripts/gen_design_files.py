...
ls_mask = lyot_mask(
    ...
    pupil_dx=pupil_dx,
    frac=LS_FRAC,
    obscuration_ratio=LS_OBSCURATION_RATIO,
    shift=True,
)
...
if np.__name__ == "cupy":
    im = ax3.imshow(
        contrast_onax.get(), cmap="inferno", norm=LogNorm(vmax=1e-5, vmin=1e-11)
    )
else:
